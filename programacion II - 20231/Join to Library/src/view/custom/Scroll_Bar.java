...
public class Scroll_Bar extends BasicScrollBarUI {
	...
	protected void paintTrack (Graphics g, JComponent c, Rectangle trackBounds) {
		g.setColor(Constants.defaultBgColor);
	...
	protected void paintThumb (Graphics g, JComponent c, Rectangle r) {
		...
		if (!sb.isEnabled() || r.width > r.height) {
			...
			color = Constants.defaultBgColor.brighter().brighter().brighter();
		} else if (isThumbRollover()) {
			color = Constants.defaultBgColor.brighter().brighter();
		} else {
			color = Constants.defaultBgColor.brighter();
		}
		g2.setPaint(color);
